...
from Transparencyattn.Trainers.DatasetBC import DataHolder, Dataset
import logging
logging.basicConfig(format='%(levelname)s - %(asctime)s - %(message)s', level=logging.INFO)

#######################################################################################################################

def process_int_grads(grads):

    for i in range(len(grads)):
        grads[i] = np.abs(grads[i]).sum(0)  #squeezes unwanted dim
        grads[i] = grads[i] / grads[i].sum()  #normalizes
